...
public class BotInviteEvent extends ListenerAdapter {
    ...
    public void onPrivateMessageReceived(PrivateMessageReceivedEvent e) {
        ...
        if (e.getMessage().getContentRaw().contains("https://discordapp.com/api/oauth2/authorize?client_id=")) {
            ...
            eB.addField("Vielen Dank für deinen BotInviteEvent", "Dein BotInviteEvent Invite wird nun zu einem Mod/Admin geschickt. Diese überprüfen deinen Status und den deines Bots. Bitte gedulde dich ...", true);
            eB.setColor(Green.g700);
            e.getChannel().sendMessage(eB.build()).complete();
            eB.clear();
            eB.addField("\uD83D\uDEE0  BotInviteEvent Invite Link", "```" + inviteLink + "```", false);
